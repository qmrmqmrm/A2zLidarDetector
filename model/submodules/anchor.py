...
class DefaultAnchorGenerator_(nn.Module):
    """
    ...
    def __init__(self, feature_shape):
        ...
        self.device = feature_shape[0].device
        self.stride = cfg.Model.Structure.STRIDE_SHAPE
        # fmt: on
        """
        ...
        self.num_features = len(self.feature_shape)
    ...
    def _calculate_anchors(self, sizes, aspect_ratios):
        ...
        cell_anchors = [self.generate_cell_anchors(sizes, aspect_ratios).to(self.device, torch.float32)]
    ...
    def grid_anchors(self, grid_sizes):
        anchors = []
        for size, stride, base_anchors in zip(grid_sizes, self.stride, self.cell_anchors):
            shift_x, shift_y = _create_grid_offsets(size, stride)
            shifts = torch.stack((shift_x, shift_y, shift_x, shift_y), dim=1).to(DEVICE, non_blocking=True)
    ...
    def make_anchor(self):
        ...
        grid_sizes = [feature_map for feature_map in self.feature_shape]
...
class DefaultAnchorGenerator(nn.Module):
    ...
    def _calculate_anchors(self, sizes, aspect_ratios):
        ...
        cell_anchors = [self.generate_cell_anchors().float() for s, a in zip(sizes, aspect_ratios)]
    ...
    def grid_anchors(self, grid_sizes):
        ...
        for size, stride, base_anchors in zip(grid_sizes, self.strides, self.cell_anchors):
            print(size,stride, base_anchors.shape)
            shift_x, shift_y = _create_grid_offsets(size, stride)
            print('sshift',shift_x.shape,shift_y.shape)
    ...
    def generate_cell_anchors(self, sizes=(16, 64, 80), aspect_ratios=(0.5, 1, 2)):
        """
        Generate a tensor storing anchor boxes, which are continuous geometric rectangles
        centered on one feature map point sample. We can later build the set of anchors
        for the entire feature map by tiling these tensors; see `meth:grid_anchors`.
    ...
    def forward(self, features):
        ...
        Returns:
            list[list[Boxes]]: a list of #image elements. Each is a list of #feature level Boxes.
                The Boxes contains anchors of this image on the specific feature level.
        """
        # [[2, 256, 160, 160], [2, 256, 80, 80], [2, 256, 40, 40]]
