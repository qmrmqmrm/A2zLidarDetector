...
class Anchor:
    ...
    def __call__(self):
        ...
        """
        anchors = []
        for feature_i, (lw_shape, size, stride) in enumerate(zip(self.feature_shapes, self.anc_sizes, self.strides)):
            anchor_per_scale = len(self.aspect_ratio)
            zeros = torch.zeros((lw_shape[0], lw_shape[1], anchor_per_scale, 2), device=self.device)
            anc_area = size ** 2
            ...
            pos_y = torch.arange(0, lw_shape[0] * stride, step=stride, dtype=torch.float32,
                                 device=self.device) + 0.5 * (stride - 1)
            pos_x = torch.arange(0, lw_shape[1] * stride, step=stride, dtype=torch.float32,
                                 device=self.device) + 0.5 * (stride - 1)
            mesh_y, mesh_x = torch.meshgrid(pos_y, pos_x)
            pos_yxs = torch.stack([mesh_y, mesh_x], -1).view((lw_shape[0], lw_shape[1], 1, 2))
            ...
            anchor_id = anchor_id.repeat(lw_shape[0], lw_shape[1], 1).unsqueeze(-1)
            stride = torch.ones(lw_shape[0], lw_shape[1], 3, 1) * stride
            anchor_box2d = torch.cat((pos_yxs, anchor_hws, anchor_id, stride), dim=-1)
