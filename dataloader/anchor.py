...
class Anchor:
    def __init__(self):
        sizes = _broadcast_params(cfg.Model.RPN.ANCHOR_SIZES, 3, "sizes")
        aspect_ratios = _broadcast_params(cfg.Model.RPN.ANCHOR_RATIOS, 3, "aspect_ratios")
    ...
    def generate_cell_anchors(self, sizes, aspect_ratios):
        ...
        return torch.tensor(anchors, device="cuda")
    ...
    def make_anchor_map(self, heights):
        """
        Args:
        ...
        for height in heights:
            stride.append(torch.zeros(height, height * 2).size())
        anchors_over_all_feature_maps = self._grid_anchors(stride)
