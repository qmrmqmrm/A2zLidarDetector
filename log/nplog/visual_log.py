...
class VisualLog:
    ...
    def __call__(self, step, grtr, gt_feature, pred, pred_nms):
        ...
        for batch_idx in range(batch):
            ...
            scale_img = self.draw_ctgr_boxes(scale_img, splits["pred_tp"], batch_idx, 150)
            scale_img = self.draw_ctgr_boxes(scale_img, splits["pred_fp"], batch_idx, 150)
            scale_img = self.draw_ctgr_boxes(scale_img, splits["grtr_tp"], batch_idx, 255)
            scale_img = self.draw_ctgr_boxes(scale_img, splits["grtr_fn"], batch_idx, 255)
    ...
    def draw_ctgr_boxes(self, image, bboxes, frame_idx, val):
        """
        all input arguments are numpy arrays
        :param image: (H, W, 3)
        :param bboxes: {'yxhw': (B, N, 4), 'category': (B, N, 1), ...}
        ...
        :return: box drawn image
        """
        shape = bboxes["category"].shape
        if shape[-1] >1:
            best_cate = np.argmax(bboxes["category"], axis=-1)
