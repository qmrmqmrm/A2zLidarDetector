...
class VisualLog:
    ...
    def __call__(self, step, grtr, pred, grtr_feat, pred_feat):
        ...
        for batch_idx in range(batch):
            ...
            cam_image = self.get_cam_image(grtr, batch_idx)
            bev_image = self.load_image(grtr, batch_idx)
    ...
    def rotate_box3d(self, splits3d):
        ...
        for key in splits_keys:
            ...
            splits3d[key]['bbox3d'] = np.stack(rot_bbox3d_per_img, axis=0)
        return splits3d
    def get_cam_image(self, grtr, batch_idx):
    ...
    def load_image(self, grtr, batch_idx, white=False):
    ...
    def draw_objects(self, img, splits, draw_func, put_text, val):
        text_splits = {key: splits[key] for key in put_text}
    ...
    def concat_and_save_image(self, step, img, grtr_img, pred_img, batch, batch_idx, title):
        vlog_image = np.concatenate([grtr_img, pred_img], axis=1)
        vlog_image = np.concatenate([img, vlog_image], axis=0)
        filename = op.join(self.vlog_path, f"{step * batch + batch_idx:05d}_{title}.jpg")
        cv2.imwrite(filename, vlog_image)
