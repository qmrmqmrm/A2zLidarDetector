...
def merge_and_slice_features(features, is_gt):
    ...
    """
    scales = ['feature_l', 'feature_m', 'feature_s']  # ['feature_l', 'feature_m', 'feature_s']
    sliced_features = {}
    for key in scales:
    ...
    if "dontcare" in features:
        bbox_compos = cfg.Model.Output.get_bbox_composition(is_gt)
        sliced_features["dontcare"] = slice_feature(features["dontcare"], bbox_compos)

    if model_cfg.TRAIN_LANE:
        if "feat_lane" in features:
            merged_feat = merge_dim_hwa(features["feat_lane"])
            lane_compos = cfg.Model.Output.get_lane_composition(is_gt)
            sliced_features["feat_lane"] = slice_feature(merged_feat, lane_compos)

        if "lanes" in features:
            lane_compos = cfg.Model.Output.get_lane_composition(is_gt)
            sliced_features["lanes"] = slice_feature(features["lanes"], lane_compos)
